...
GET_IP(){
    ...
    if [ $ip_type = "public" ]
    then
        aws ec2 describe-instances --instances-id $instance_id --query 'Reservations[0].Instances[0].PublicIpAddress' --output text
    else
        aws ec2 describe-instances --instances-id $instance_id --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text
    fi
}
